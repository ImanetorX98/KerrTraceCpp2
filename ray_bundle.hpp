...
static BundleResult trace_bundle(int px, int py,
    ...
    const double observer_frequency = -bs.geo.pt * g.static_observer_ut(cam.r_obs, cam.theta_obs);
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        if (maybe_equator) {
            ...
            if (crossed_equator && !see_through &&
                ...
                // Same invariant frequency ratio as the single-ray tracers.
                const double red = g.disk_frequency_shift(
                    r_hit, bs.geo.pt, bs.geo.pphi, observer_frequency);
